...
class UNet(nn.Module):
  def __init__(self, num_blocks,filter_sequence,max_pool_sequence, dilation_list, num_classes = 2):
    ...
    #dilation_list: list of dilation parameters for the residual blocks
    ...
    for i in range(num_blocks):
        self.downs.append(ResidualBlock(in_channels,filter_sequence[i],3,3,dilation_list[i]))
    ...
    for i in reversed(range(num_blocks)):
        self.ups.append(ResidualBlock(filter_sequence[i]+filter_sequence[i+1],filter_sequence[i],3,3,dilation_list[i])) #The 2*filters in the input channels refers to the extra channels from the concat layer
        self.upsamples.append(nn.Upsample(scale_factor=max_pool_sequence[i]))

    # "bottleneck" or middle part at bottom of U
    self.bottleneck = ResidualBlock(filter_sequence[num_blocks-1],filter_sequence[num_blocks],3,3,dilation_list[num_blocks])
...
pool_sequence = [2,2] # Sets the max pooling kernel size. The length of this list has to be equal to num_blocks
dil_list = [1,1,2] # Sets the dilation parameters for the num_blocks+1 residual blocks
dil_str = ''.join(str(i) for i in dil_list)
...
for seed_num in seed_list:
     ...
     model = UNet(num_blocks,filter_sequence,pool_sequence,dil_list)
     ...
     torch.save(model, 'UNet_b2_Au_d'+dil_str+'_run'+str(i)+'_model.h5')
     history_dataframe = DataFrame(history)
     hist_filename ='UNet_b2_Au_d'+dil_str+'_run'+str(i)+'_trainingLossHistory.csv'
